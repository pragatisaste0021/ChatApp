...
const Signup = () => {
    ...
    const confirmPassword = watch("confirmPassword", "")
      ...
      const onSubmit = async(data) => {
        ...
        await axios.post("/api/user/signup", userInfo, {withCredentials: true,}).then((response) => {
            if(response.data){
  ...
  return (
    <>
        <div className='flex h-screen justify-center items-center bg-gray-800'>
            ...
            <form action="" className='border border-black px-6 py-2 rounded-md space-y-3 w-96' onSubmit={handleSubmit(onSubmit)}>

                <h1 className='text-2xl text-center'><span className="text-gray-100">Chat</span> <span className='text-green-500 font-semibold'>App</span></h1>
                <h2 className='text-xl text-black font-bold text-gray-200'>SignUp</h2>
                ...
                <label className="input input-bordered flex items-center gap-2 !bg-gray-200">
                    ...
                    <input type="text" className="grow mt-3" placeholder="Username"  {...register("fullname", { required: true })}/>
                </label>
